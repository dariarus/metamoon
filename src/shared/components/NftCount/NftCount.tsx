...
export const NftCount: FC = () => {
  ...
  const handleIncreaseNftCount = (): void => {
  ...
  const handleDecreaseNftCount = (): void => {
    if (nftCount === 0) {
      return;
    }
    setNftCount((nftCount) => nftCount - 1);
  ...
  return (
    <div className={styles['count']}>
      <button
        ...
        onClick={handleDecreaseNftCount}
        disabled={nftCount === 0}
      ></button>
      <p className={styles['count__text']}>
        {nftCount}/{tmpMaxNftCount}
      </p>
      <button
        className={`${styles['button']} ${styles['button_increase']}`}
        onClick={handleIncreaseNftCount}
        disabled={nftCount === tmpMaxNftCount}
      ></button>
    </div>
  );
};
